...
double num_of_vr=100;
//double num_of_cluster=1;
...
{// cache storage size aware key range distribution

    int num_of_clu = 2;

for(int i=0; i<num_of_clu; i++)
{
    double left = num_of_vr;
    double sum_keys = 0;
    double num_clu_members = 0;
    double total_cache_size = 0;
    double key_dis =-1;
    cout<<"=====================cluster "<< i<<endl;
    for(int r=0; r<nodes; r++)
    {
        ...
        {
            num_clu_members +=1;
            total_cache_size += cacheini[r];
        }

    }
    cout<<"num of clu mem "<<num_clu_members<<endl;
    cout<<"total_cache_size "<<total_cache_size<<endl;
    ...
    for(int r=0; r<nodes; r++)
    {
        ...
        {
            num_clu_members -=1;
            left = left - round((num_of_vr*cacheini[r])/total_cache_size);
//            cout<<"left "<<left<<endl;
            if (left < 0)
            {
//                cout<<"----less than----"<<endl;
                left=round((num_of_vr * cacheini[r]) / total_cache_size)+left;
                key_dis=left;
                CHW[r].keys=key_dis;
//                cout<<r<<" router's key dis "<<CHW[r].id<<endl;
                sum_keys +=CHW[r].keys;
                cout<<r<<" total used keys "<<sum_keys<<endl;

            }
            else if((num_clu_members==0)&&(sum_keys+round((num_of_vr * cacheini[r]) / total_cache_size)<num_of_vr ))
            {
//                cout<<"----more than----"<<endl;
                key_dis = num_of_vr-sum_keys;
                CHW[r].keys=key_dis;
//                cout<<r<<" router's key dis "<<CHW[r].id<<endl;
                sum_keys +=CHW[r].keys;
                cout<<r<<" total used keys "<<sum_keys<<endl;
            ...
            {
//                cout<<"----normal----"<<endl;
                key_dis=round((num_of_vr*cacheini[r])/total_cache_size);
                CHW[r].keys=key_dis;
//                cout<<r<<" router's key dis "<<CHW[r].id<<endl;
                sum_keys +=CHW[r].keys;
                cout<<r<<" total used keys "<<sum_keys<<endl;
