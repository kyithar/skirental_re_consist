...
void core_layer::handle_timers(cMessage *timer){
    ...
    //normal_mean=ContentStore->get_tot_req();

    w=incoming;
//    cout<<"w"<<w<<endl;
    incoming=0;
//cout<<"reset counter"<<incoming<<endl;
scheduleAt( simTime() + check_time, timer );
}
...
void core_layer::handle_interest(ccn_interest *int_msg)
{
   ...
   chunk_t chunk = int_msg->getChunk();
	...
	//
		...
		CSTAT[chunk].req +=1;
		double p_pre;
		double gamma = uniform(0, 1);
//		B = round(ContentStore->get_tot_req());
//	    c = (pow((1 + 1/B),B))-1;
//	    p_pre =p[chunk].p;
//	    p[chunk].p = (p_pre *(1+1/B))+(1/(c*B));


if(ContentStore->full()){
//		c =round(ContentStore->get_tot_req());
//		B = round(ContentStore->get_tot_req());
//		cout<<"print c "<<c<<endl;
		if(p[chunk].p == -1){
		    p[chunk].c = round(ContentStore->get_tot_req());
		    B = p[chunk].c;
		    p_pre = 0;
	        c = pow(1 + 1/B,B)-1;
	        p[chunk].p = (p_pre *(1+1/B))+(1/(c*B));
//	        cout<<"chcek p "<<p[chunk].p<<endl;
...
//            p_pre = p[chunk].p;
//            q=pow(1+(1/p[chunk].c),p[chunk].c)-1;
//            p[chunk].p = (p_pre*(1+(1/p[chunk].c)))+1/(q*p[chunk].c);
//            q=pow(1+(1/c),p[chunk].c)-1;
//            p[chunk].p = (p_pre*(1+(1/c)))+1/(q*c);
		}
...
//
//	                U1 = CSTAT[chunk].req;
////	                cout<<"u1 req cs"<<U1<<endl;
//	                  if(ContentStore->full()){
//	                      U2=ContentStore->get_tot_req();
////	                      cout<<"tot req cs"<<U2<<endl;
//	                  }
////	                  }else{
////	                        U2=1;
////	                  }
...
//	                double tmp_nu = 1.0/(1.0+exp((-k)*theta));
//	                CSTAT[chunk].nu = tmp_nu;
////	                tmp_nu =1.0/(1.0+exp((-k)*tmp_nu));
////	                tmp_nu =1.0/(1.0+exp((-k)*tmp_nu));
////	                CSTAT[chunk].nu = 1.0/(1.0+exp((-k)*tmp_nu));
//	                //cout<<"U1 "<<U1<<"---U2 "<<U2<<" ---nu "<<CSTAT[chunk].nu<<endl;
//	                double tmp_ran=uniform(0, 1);
//
////	                if(CSTAT[chunk].nu>=1){
////	                           CSTAT[chunk].CB=1;
////	                          // cout<<"remove U2 and cache U1 with thershold"<<tmp_ran<<endl;
////	                       }
//
//	                if(CSTAT[chunk].nu>tmp_ran){
//	                    CSTAT[chunk].CB=1;
//	                   // cout<<"remove U2 and cache U1 with thershold"<<tmp_ran<<endl;
//	                }
//	                }
...
void core_layer::handle_data(ccn_data *data_msg)
{
	...
	{
		if(p[chunk].p==1){
		    data_msg->setCur_req(CSTAT[chunk].req);
		    ContentStore->store(data_msg);
		    p.erase(chunk);
		    CSTAT.erase(chunk);
		}
